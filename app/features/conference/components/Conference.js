...
class Conference extends Component<Props, State> {
    ...
    _loadConference() {
        ...
        //Create Token
        var token=this._setToken(this.props._appId,this.props._appSecret,this.props._avatar,this.props._host,this.props._name,this.props._email);
        const options = {
            configOverwrite,
     ...
     * @private
     */
    _setToken(appId:string,appKey:string,avatar:string,host:string,name:string,email:string)
    {
        return jwt.sign(
            {
                // token数据
